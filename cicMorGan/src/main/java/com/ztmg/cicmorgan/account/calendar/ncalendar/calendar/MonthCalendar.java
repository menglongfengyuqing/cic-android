...
import com.ztmg.cicmorgan.account.calendar.ncalendar.adapter.MonthAdapter;
import com.ztmg.cicmorgan.account.calendar.ncalendar.listener.OnClickMonthViewListener;
import com.ztmg.cicmorgan.account.calendar.ncalendar.listener.OnMonthCalendarChangedListener;
...
public class MonthCalendar extends CalendarPager implements OnClickMonthViewListener {
    ...
    protected void initCurrentCalendarView(int position) {
        ...
        if (lastPosition == -1) {
            ...
            if (isDefaultSelect) {
                ...
                currView.setDateTimeAndPoint(mSelectDateTime.minus(i), pointList, true);
                if (onMonthCalendarChangedListener != null) {
                    onMonthCalendarChangedListener.onMonthCalendarChanged(mSelectDateTime);
                }
            } else {
    ...
    protected void setDateTime(DateTime dateTime) {
        ...
        currectMonthView.setDateTimeAndPoint(dateTime, pointList, false);
    ...
    private void dealClickEvent(DateTime dateTime, int currentItem) {
        ...
        if (dateTime.equals(mInitialDateTime)) {
            nMonthView.setDateTimeAndPoint(dateTime, pointList, false);
        } else {
            nMonthView.setDateTimeAndPoint(dateTime, pointList, true);
        }
        mSelectDateTime = dateTime;
        lastSelectDateTime = dateTime;
        ...
        if (onMonthCalendarChangedListener != null) {
            onMonthCalendarChangedListener.onMonthCalendarClickChanged(dateTime);
        }
    }
