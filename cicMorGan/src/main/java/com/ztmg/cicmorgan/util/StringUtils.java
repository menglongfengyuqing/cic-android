...
public class StringUtils {
    ...
    */
        String num = "[1][12345678]\\d{9}";//"[1]"代表第1位为数字1，"[358]"代表第二位可以为3、5、8中的一个，"\\d{9}"代表后面是可以是0～9的数字，有9位。
     ...
     * 获取高亮字符串
     */
    public static String getHighLightString(String highLight, String title) {

        String latsTitle = null;
        try {
            latsTitle = "";
            if (highLight.contains("&")) {
                String[] result = highLight.split("&");
                String subColorStr = null, substr = null;

                for (int i = 0; i < result.length; i++) {
                    if (i == 1) {
                        subColorStr = "<font color=\"#C79E63\">" + result[i] + "</font>";
                        substr = result[i];
                    }
                }
                latsTitle = title.replaceFirst(substr, subColorStr);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
