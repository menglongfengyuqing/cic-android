...
import com.ztmg.cicmorgan.net.testA.exception.MyException;
import com.ztmg.cicmorgan.net.testA.util.LogUtils;
...
public abstract class JsonCallback<T> extends AbsCallback<T> {
    ...
    public void onError(com.lzy.okgo.model.Response<T> response) {
        ...
        if (response.getException() instanceof SocketTimeoutException) {
            ...
            switch (((MyException) response.getException()).getErrorBean().Code) {
                case "4"://约定的身份表示过期
                    //重登录  手势密码
                    String mGesture = LoginUserProvider.getUser(MyApplication.getInstance()).getGesturePwd();
                    if (mGesture.equals("1") && !mGesture.equals("") && mGesture != null) {// 判断是否设置手势密码
                        // 设置手势密码
                        Intent intent = new Intent(MyApplication.getInstance(), UnlockGesturePasswordActivity.class);
    ...
    public interface HttpCallback {
        void success(String result, String whereRequest);
